...
def declare_logic():
    ...
    # Derive Customer.total_spent as the sum of Order.total_amount for each customer.
    Rule.sum(derive=Customer.total_spent, as_sum_of=Order.total_amount, where=None)

    # Derive Customer.total_pets as the count of pets owned by the customer.
    Rule.count(derive=Customer.total_pets, as_count_of=Pet, where=None)

    # Derive OrderItem.item_total as the product of quantity and product price.
    Rule.formula(derive=OrderItem.item_total, as_expression=lambda row: row.quantity * row.product.price)

    # Derive Order.total_amount as the sum of all OrderItem.item_total for each order.
    Rule.sum(derive=Order.total_amount, as_sum_of=OrderItem.item_total, where=None)
